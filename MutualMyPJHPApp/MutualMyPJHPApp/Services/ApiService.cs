namespace MutualMyPJHPApp.Services
{
    ...
    public class ApiService
    {
        ...
        //    try
        //    {
        //        var userRequest = new UserRequest { USRLOGIN = email, };
        ...
        public async Task<Response> GetUserByEmail(string urlBase, string servicePrefix, string controller, string email)
        {
            try
            {
                var userRequest = new UserRequest { USRLOGIN = email };
        ...
        public async Task<Response> GetList<T>(string urlBase, string Prefix, string controller, string tokenType, string accessToken, int id)
        {
            try
            {
                var client = new HttpClient();
                client.BaseAddress = new Uri(urlBase);
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(tokenType, accessToken);
                var url = string.Format("{0}{1}/{2}", Prefix, controller, id);
                var response = await client.GetAsync(url);
                var answer = await response.Content.ReadAsStringAsync();
                if (!response.IsSuccessStatusCode)
                {
